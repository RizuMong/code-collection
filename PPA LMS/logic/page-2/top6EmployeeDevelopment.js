...
const data = _query(`
        SELECT employee_id_text, SUM(training_hour) as total
        ...
        GROUP BY employee_id_text
        ORDER BY total DESC
        LIMIT 6;
    `)

data.forEach(row => {
    const employee = _find("[master_data]t_employee", {
        id_employee: row.employee_id_text
    })
