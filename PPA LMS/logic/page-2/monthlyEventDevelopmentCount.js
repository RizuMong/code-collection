...
months.forEach(month => {
    date_start_of_month = month.logic_getTimeStamp('start')
    date_end_of_month = month.logic_getTimeStamp('end')

    const count = _query(`
        SELECT COUNT(mds_id_text) AS event_count
        FROM t_training_participant
        WHERE is_completed_text = 'YES'
        AND date_start_plan >= ${date_start_of_month}
        AND date_start_plan <= ${date_end_of_month}
