...
for (var u = 0; u < get_users.length; u++) {
  var user = get_users[u];
...
grouped["UNASSIGNED"] = {
  user_id: { id: "UNASSIGNED", name: "Unassigned Task" },
  job_position_id: {},
  capacity_manhours: 0,
  leave_manhours: 0,
  planned_manhours: 0,
...
for (var i = 0; i < get_datas.length; i++) {
  ...
  var assigneeId = (item.assignee_id && item.assignee_id.id)
    ? item.assignee_id.id
    : "UNASSIGNED";

  if (!grouped[assigneeId]) continue;
...
for (var key in grouped) {
  if (grouped.hasOwnProperty(key)) {
    ...
    if (key === "UNASSIGNED") {
      obj.status = obj.planned_manhours > 0 ? "Unassigned" : "No Task";
