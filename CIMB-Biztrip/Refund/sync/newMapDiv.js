...
findLogs.forEach(function (log) {
  if (log.activity.id === data.id) {
    var divisionId = log.division.id;
    if (!divisionMap[divisionId]) {
      divisionMap[divisionId] = {
        division_id: divisionId,
        division_name: log.division.name,
        amount: 0,
        refund_amount: 0,
        rc_code: log.rc_code,
    ...
    divisionMap[divisionId].amount += log.transaction_amount || 0;
    divisionMap[divisionId].refund_amount += log.refund_amount || 0;
  }
});

// Convert the map to an array
var pureDivision = Object.keys(divisionMap).map(function (key) {
  return divisionMap[key];
});

console.log({ pureDivision: pureDivision });
